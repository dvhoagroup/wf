...
namespace it
{
    public class CommonCls
    {
        ...
        public static void ExportExcel(DataTable table)
        {
            ...
            {
                var fileName = $"banchothue_error{DateTime.Now.ToString("ddMMyyyyHHmm")}.xls";
                ...
                for (int i = 2; i < table.Rows.Count + 2; i++)
                {
                    ...
                    if (item["TrangThaiImport"].ToString() == "Thông tin đã có trên hệ thống HOALAND")
                    {
                        sheet.Range[$"C{i}:G{i}"].Style.Color = System.Drawing.Color.Red;
                        sheet.Range[$"AI{i}:AL{i}"].Style.Color = System.Drawing.Color.Red;
                    }
                    else if (item["TrangThaiImport"].ToString() == "Khách hàng lấy từ hệ thống")
                    {
                        sheet.Range[$"AI{i}:AL{i}"].Style.Color = System.Drawing.Color.Green;
                    }
                }
