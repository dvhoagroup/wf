...
public class BaoKimPayment
{
    ...
    //Mã merchant site
    private string merchant_id = "3447";	//Thay bằng mã merchant site bạn đã đăng ký trên BaoKim.vn

    //Mật khẩu bảo mật
    private string secure_pass = "79f3326e99fd7708";	//Thay bằng mật khẩu giao tiếp giữa website của bạn với BaoKim.vn
    ...
    public String createRequestUrl(String order_id, String business, String total_amount, String shipping_fee, String tax_fee, String order_description, String url_success, String url_cancel, String url_detail)
    {
        ...
        foreach (string key in keys)
        {
            Object value = order_params[key];
            if (url_params == "")
                url_params += key.ToString() + "=" + System.Web.HttpUtility.UrlEncode(value.ToString());
            else
                url_params += "&" + key.ToString() + "=" + System.Web.HttpUtility.UrlEncode(value.ToString());
        }
