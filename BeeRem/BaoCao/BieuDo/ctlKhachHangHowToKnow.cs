...
{
    public partial class ctlKhachHangHowToKnow : DevExpress.XtraEditors.XtraUserControl
    {
        public ctlKhachHangHowToKnow()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                using (var db = new MasterDataContext())
                {
                    var ltHTK = (from kh in db.KhachHangs
                                 join htk in db.cdHowToKnows on kh.HowToKnowID equals htk.ID
                                 group htk by new { htk.ID, htk.Name} into gr
                                   select new 
                                   {
                                       HTK = gr.Key.Name,
                                       SoLuong = gr.Count()
                    ...
                    var strTitle = "Chart Of Customers By How To Know";

                    Series series1 = new Series(strTitle, ViewType.Doughnut);

                    // Add points to the series.
                    foreach (var dt in ltHTK)
                        series1.Points.Add(new SeriesPoint(string.Format("{0} ({1})", dt.HTK, dt.SoLuong), dt.SoLuong));
