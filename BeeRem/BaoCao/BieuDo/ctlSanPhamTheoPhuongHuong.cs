...
{
    public partial class ctlSanPhamTheoPhuongHuong : DevExpress.XtraEditors.XtraUserControl
    {
        ChartControl barChart3D;
        string Zone = "";
        int Amount = 0;
        List<Data> objData;
        public ctlSanPhamTheoPhuongHuong()
        {
            InitializeComponent();
        }
        ...
        void FullStackedBar()
        {
            ...
            using (var db = new MasterDataContext())
            {
                try
                {
                    var temp = (from sp in db.bdsSanPhams
                                join pht in db.PhuongHuongs on sp.MaHuong equals pht.MaPhuongHuong
                                where sp.MaDA ==  (int?)itemDuAn.EditValue
                                group new { pht, sp } by new { sp.MaHuong, pht.TenPhuongHuong } into gr
                                select new
                                {
                                    MaHuong = gr.Key.MaHuong,
                                    TenHuong = gr.Key.TenPhuongHuong,
                    ...
                    foreach (var s in temp)
                    {
                        ...
                        series1.Points.Add(new SeriesPoint(string.Format("{0}", s.TenHuong), s.DaBan));
                       // var val2 = db.funcCountAgentSoldByProject(s.MaDA);
                        series2.Points.Add(new SeriesPoint(string.Format("{0}", s.TenHuong), s.ChuaBan));
            ...
            chartTitle1.Text = "THAM CHIẾU GIAO DỊCH THEO HƯỚNG";
