...
{
    public partial class ctlResutlBuy : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        void FullStackedBar()
        {
            ...
            using (var db = new MasterDataContext())
            {
                try
                {
                    ...
                    {
                        var val1 = db.funcCountByZone(s.MaKhu, 1);
                        series1.Points.Add(new SeriesPoint(string.Format("Khu {0}", s.TenKhu), val1));
                        var val2 = db.funcCountByZone(s.MaKhu, 2);
                        series2.Points.Add(new SeriesPoint(string.Format("Khu {0}", s.TenKhu), val2));
                        var val3 = db.funcCountByZone(s.MaKhu, 3);
                        series3.Points.Add(new SeriesPoint(string.Format("Khu {0}", s.TenKhu), val3));

                        var item = new Data(index, string.Format("Khu {0}", s.TenKhu), (int)val1, (int)val2, (int)val3);
                        objData.Add(item);
                        index++;
                    }
                    Amount = zone.Count;
            ...
            FullStackedBarSeriesView myView3 = (FullStackedBarSeriesView)series3.View;
            myView3.Transparency = 50; myView3.BarWidth = GetBarWidth();
        ...
        void Pie3D()
        {
            try
            {
                ...
                using (var db = new MasterDataContext())
                {
                    try
                    {
                        series1.Points.Add(new SeriesPoint("Đã bán", db.funcCountByProject((int)itemDuAn.EditValue, 1)));
                        series1.Points.Add(new SeriesPoint("Đặt cọc", db.funcCountByProject((int)itemDuAn.EditValue, 2)));
                        series1.Points.Add(new SeriesPoint("Trống", db.funcCountByProject((int)itemDuAn.EditValue, 3)));
                    }
                    catch { }
                }
