...
namespace LandSoft.CongVan.received
{
    public partial class ctlManager : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ctlManager()
        {
            ...
            lookLoaiCV.DataSource = db.LoaiCongVanDens;
        ...
        void Permission()
        {
            try
            {
                var listAction = db.ActionDatas.Where(p => p.FormID == 135 & p.PerID == Properties.Settings.Default.PerID)
        ...
        void LoadData()
        {
            ...
            try
            {
                ...
                switch (it.CommonCls.GetAccessData(Properties.Settings.Default.PerID, 135))
                {
                    case 1: gcCongVan.DataSource = db.CongVanDen_getDate(tuNgay, denNgay, 0, 0, 0);
                        break;
                    case 2://Theo phong ban
                        gcCongVan.DataSource = db.CongVanDen_getDate(tuNgay, denNgay, Properties.Settings.Default.DepartmentID, 0, 0);
                        break;
                    case 3://Theo nhom
                        gcCongVan.DataSource = db.CongVanDen_getDate(tuNgay, denNgay, 0, Properties.Settings.Default.GroupID, 0);
                        break;
                    case 4://Theo nhan vien
                        gcCongVan.DataSource = db.CongVanDen_getDate(tuNgay, denNgay, 0, 0, Properties.Settings.Default.StaffID);
                        break;
                   default:
        ...
        void Delete()
        {
            ...
            using (var db = new MasterDataContext())
            {
                try
                {
                    ...
                    {
                        var bg = db.CongVanDens.Single(p => p.ID == (int)gvCongVan.GetRowCellValue(i, "ID"));
                        db.CongVanDens.DeleteOnSubmit(bg);
                    }
                    db.SubmitChanges();
                    LoadData();
                }
        ...
        void Export()
        {
            ...
            frm.FileName = "TK Danh sach cong van den -- " + DateTime.Now.ToLongDateString();
        ...
        void Clicks()
        {
            if (gvCongVan.FocusedRowHandle >= 0)
            {
                ...
                {
                    case 0:
                        gcNhatKy.DataSource = db.cvdNhatKyXuLies.Where(p => p.MaCV == MaCV).OrderByDescending(p => p.NgayXL).AsEnumerable()
                    .Select((p, index) => new
                    {
                        STT = index + 1,
                        p.ID,
