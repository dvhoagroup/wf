...
{
    public partial class ctlManagerChoXoa : DevExpress.XtraEditors.XtraUserControl
    {
        MasterDataContext db = new MasterDataContext();
        mglbcBanChoThue objGD { get; set; }

        public ctlManagerChoXoa()
        {
            InitializeComponent();
        }
        ...
        int GetAccessData()
        {
            it.AccessDataCls o = new it.AccessDataCls(Common.PerID, 20);
                ...
                switch (GetAccessData())
                {
                    case 1://Tat ca
                        ...
                        if (obj.DienThoai == false)
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, -1, -1, -1, -1, MaNV, true)
                                .Select(p => new
                                {
                                    p.STT,
                                    p.MaBC,
                            ...
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, -1, -1, -1, -1, MaNV, true).ToList();
                        #endregion
                        break;
                    case 2://Theo phong ban 
                        #region phong ban
                        if (obj.DienThoai == false)
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, -1, -1, -1, Common.DepartmentID, MaNV, false)
                                .Select(p => new
                                {
                                    p.STT,
                                    p.MaBC,
                            ...
                            db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, -1, -1, -1, Common.DepartmentID, MaNV, false).ToList();
                        #endregion
                        break;
                    case 3://Theo nhom
                        #region Theo nhom
                        if (obj.DienThoai == false)
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, -1, -1, Common.GroupID, -1, MaNV, false)
                                .Select(p => new
                                {
                                    p.STT,
                                    p.MaBC,
                            ...
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, -1, -1, Common.GroupID, -1, MaNV, false).ToList();//.Where(p => p.IsBan.GetValueOrDefault() == true);
                        #endregion
                        break;
                    case 4://Theo nhan vien
                        #region Theo nhan vien
                        if (obj.DienThoai == false)
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, MaNV, MaNV, -1, -1, MaNV, false)
                                .Select(p => new
                                {
                                    p.STT,
                                    p.MaBC,
                            ...
                            gcBan.DataSource = db.mglbcBanChoThue_getByDateChoXoa(tuNgay, denNgay, arrMaTT, true, MaNV, MaNV, -1, -1, MaNV, false).ToList();//.Where(p => p.IsBan.GetValueOrDefault() == true);
                        #endregion
                        break;
                    default:
                        gcBan.DataSource = null;
        ...
        private void ctlManagerChoXoa_Load(object sender, EventArgs e)
        {
        ...
        private void itemKhongDuyet_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (DialogBox.QuestionNotApproved() == DialogResult.No) return;
            db = new MasterDataContext();
            foreach (var i in indexs)
            {
                bool ck = (GetAccessData() == 1 || (int)grvBan.GetRowCellValue(i, "MaNVQL") == Common.StaffID) || ((int)grvBan.GetRowCellValue(i, "MaNVKD") == Common.StaffID);
