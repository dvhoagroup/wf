...
{
    public partial class ctlTrangThaiMT : XtraUserControl
    {
        MasterDataContext db = new MasterDataContext();
        public ctlTrangThaiMT()
        {
            InitializeComponent();
        ...
        void LoadData()
        {
            try
            {
                ...
                gridControl1.DataSource = db.mglmtTrangThais.OrderBy(p => p.STT).ToList();
            }
            catch
            {
        ...
        {
            frmTrangThaiMT frm = new frmTrangThaiMT();
            frm.flagAddnew = true;
            frm.ShowDialog();
            if (frm.IsUpdate)
                LoadData();
            ...
            if (grvTrangThaiGD.GetFocusedRowCellValue(colMaHuong) != null)
            {
                if (DialogBox.Question("Bạn có chắc chắn muốn xóa trạng thái: <" + grvTrangThaiGD.GetFocusedRowCellValue(colTenHuong).ToString() + "> ra khỏi hệ thống không?") == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        var obj = db.mglmtTrangThais.FirstOrDefault(p => p.MaTT == matt);
                        db.mglmtTrangThais.DeleteOnSubmit(obj);
                        db.SubmitChanges();
        ...
        private void ctlTrangThaiMT_Load(object sender, EventArgs e)
        {
